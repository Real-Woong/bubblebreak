...
export async function requestDeep3UnlockRoute(request: Request, env: Env): Promise<Response> {
  try {
    const url = new URL(request.url);
    const match = url.pathname.match(/^\/rooms\/([^/]+)\/deep3-request$/);
    ...
    if (targetUserId === session.userId) {
      return jsonResponse({ ok: false, message: "Cannot request your own private bubble" }, 409);
    }
    ...
    if (!matchedInterest || matchedInterest.level !== "deep3") {
      return jsonResponse({ ok: false, message: "Private interest not found" }, 404);
    }
    ...
    if (existing) {
      return jsonResponse({ ok: true, eventId: existing.id, status: "pending" });
    }
    ...
    const result = await env.DB.prepare(
      ...
      VALUES (?, ?, 'deep3_request', ?, ?, ?, 'pending', ?, NULL)
      `
    )
      .bind(eventId, room.id, session.userId, targetUserId, interestId, now)
      .run();

    if (!result.success) {
      return jsonResponse({ ok: false, message: "Failed to create deep3 request" }, 500);
    }
