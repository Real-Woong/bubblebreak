...
export async function approveDeep3UnlockRoute(request: Request, env: Env): Promise<Response> {
  try {
    const url = new URL(request.url);
    const match = url.pathname.match(/^\/rooms\/([^/]+)\/deep3-request\/([^/]+)\/approve$/);
    ...
    if (event.target_user_id !== session.userId) {
      return jsonResponse({ ok: false, message: "Only target can approve" }, 403);
    }
    ...
    return jsonResponse({ ok: true, eventId, status: "accepted" });
  } catch (error) {
    console.error(error);
